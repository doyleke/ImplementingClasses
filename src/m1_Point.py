...
class Point(object):
    ...
    def move_to(self, dx, dy):
        self.x = dx
        self.y = dy
        self.times_moved = self.times_moved + ((dx-self.x)+(dy-self.y))

    def move_by(self, dx, dy):
        self.x = self.x + dx
        self.y = self.y + dy
        self.times_moved = self.times_moved + (())

    def get_number_of_moves_made(self):
        return self.times_moved

    def get_distance_from(self, new_point):
        x_distance = (self.x - new_point.x)**2
        y_distance = (self.y - new_point.y)**2
        distance = math.sqrt(x_distance + y_distance)
        self.distance_moved = self.distance_moved + distance
        return distance

    def get_distance_from_start(self):
        x_distance = (self.x - self.xi) ** 2
        y_distance = (self.y - self.yi) ** 2
        distance = math.sqrt(x_distance + y_distance)
        self.distance_moved = self.distance_moved + distance
        return distance
